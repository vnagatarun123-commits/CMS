...
function SettlementStatusBadge({ status }: { status: SettlementStatus }) {
  const cfg: Record<SettlementStatus, { cls: string; label: string }> = {
    ...
    on_hold:        { cls: 'bg-red-50 text-red-600 border-red-200',         label: 'On Hold' },
    partially_paid: { cls: 'bg-orange-50 text-orange-700 border-orange-200', label: 'Partial' },
  }
...
function RedemptionStatusBadge({ status }: { status: RedemptionRequest['status'] }) {
  ...
  return <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-semibold ${cfg[status]}`}>{status.charAt(0).toUpperCase() + status.slice(1)}</span>
}
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-xl border border-border overflow-hidden">
        <div className="bg-muted/20 px-4 py-2.5 border-b border-border">
          <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wide">Content Breakdown</p>
        </div>
          ...
          {rows.map(r => {
            ...
            return (
              <div key={r.label} className="flex items-center gap-3 px-4 py-3">
                ...
                <span className="text-[11px] text-muted-foreground">{r.posts} posts</span>
                <span className="text-xs font-semibold text-foreground w-20 text-right">{inr(r.earnings)}</span>
              </div>
            )
          })}
        </div>
        <div className="flex items-center gap-3 px-4 py-2.5 bg-muted/10 border-t border-border">
          <span className="text-xs text-muted-foreground flex-1">Subtotal</span>
          <span className="text-xs font-bold text-foreground">{inr(b.imageEarningsInr + b.videoEarningsInr + b.shortEarningsInr + b.liveEarningsInr)}</span>
        </div>
      </div>
      ...
      {bonuses.length > 0 && (
        <div className="rounded-xl border border-border overflow-hidden">
          <div className="bg-muted/20 px-4 py-2.5 border-b border-border">
            <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wide">Bonuses</p>
          </div>
          <div className="divide-y divide-border">
            {bonuses.map(bon => {
              const Icon = bon.icon
              return (
                <div key={bon.label} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <span className="text-xs text-foreground flex-1">{bon.label}</span>
                  <span className="text-xs font-semibold text-emerald-600">{inr(bon.amount)}</span>
                </div>
              )
            })}
          </div>
      ...
      <div className="rounded-xl border border-border bg-muted/10 px-4 py-2.5 flex items-center justify-between">
        <span className="text-xs font-semibold text-muted-foreground">Total Impressions</span>
        <span className="text-xs font-bold text-foreground">{b.totalImpressions.toLocaleString('en-IN')}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-background rounded-xl border border-border shadow-2xl w-[380px] p-5">
        <h3 className="text-sm font-semibold text-foreground mb-1">Mark as Paid</h3>
        <p className="text-xs text-muted-foreground mb-4">
          Recording payment of <span className="font-bold text-foreground">{inr(settlement.netPayableInr)}</span> to <span className="font-medium text-foreground">{settlement.reporterName}</span> for {settlement.periodLabel}.
        </p>
        <div className="space-y-3">
          <div>
            <p className="text-[11px] text-muted-foreground mb-1">Payment Method</p>
            <div className="flex gap-2">
              {(['upi', 'bank_transfer'] as const).map(m => (
                <button key={m} onClick={() => setMethod(m)}
                  className={`flex-1 py-2 rounded-lg border text-xs font-medium transition-colors
                    ${method === m ? 'border-foreground bg-muted/30 text-foreground' : 'border-border text-muted-foreground'}`}>
          ...
          <div>
            ...
            <input value={ref} onChange={e => setRef(e.target.value)}
              ...
              className="w-full h-9 rounded-lg border border-border bg-muted/20 px-3 text-sm focus:outline-none focus:ring-1 focus:ring-foreground/20" />
          </div>
          <div>
            <p className="text-[11px] text-muted-foreground mb-1">Payment Date</p>
            <input type="date" value={date} onChange={e => setDate(e.target.value)}
              className="w-full h-9 rounded-lg border border-border bg-muted/20 px-3 text-sm focus:outline-none" />
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex items-start justify-between mb-6 px-1">
        <div>
          <h1 className="text-xl font-semibold text-foreground">Earnings Management</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Track earnings, manage settlements, and process payouts — all in ₹</p>
        </div>
      ...
      {activeTab === 'settlements' && (
        <>
          ...
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3">
              ...
              {monthIdx === MONTHS.length - 1 && (
                <span className="text-[11px] font-semibold text-blue-600 bg-blue-50 border border-blue-200 rounded-full px-2 py-0.5">Current month</span>
              )}
            </div>
            {/* Status filter */}
          ...
          <div className="grid grid-cols-5 gap-3 mb-5">
            {[
              { label: 'Gross Earnings',  value: inr(totalGross),   color: 'text-foreground',   bg: 'bg-muted/50' },
              { label: 'Net Payable',     value: inr(totalNet),     color: 'text-foreground',   bg: 'bg-muted/50' },
              { label: 'Settled',         value: inr(totalSettled), color: 'text-emerald-600',  bg: 'bg-emerald-50' },
              { label: 'Pending',         value: inr(totalPending), color: 'text-amber-700',    bg: 'bg-amber-50' },
              { label: 'TDS Deducted',    value: inr(totalTds),     color: 'text-blue-600',     bg: 'bg-blue-50' },
            ].map(s => (
              <div key={s.label} className={`rounded-xl border border-border px-4 py-3 ${s.bg}`}>

...
// ── Recent activity — relative dates so the dashboard always shows a live
//    7-day pulse, current review queue and active live streams. ──────────────
const _seedNow = new Date()
const daysAgo = (days: number, hour = 10): Date => {
  const d = new Date(_seedNow); d.setDate(d.getDate() - days); d.setHours(hour, 0, 0, 0); return d
}
const _REPS = {
